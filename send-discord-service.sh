...
json_data="{\"content\":\"${NAGIOS_SERVICEDESC} on ${NAGIOS_HOSTNAME} ${NAGIOS_SERVICESTATE}\",
    \"username\":\"${NAGIOS_NAME}\",
    \"avatar_url\":\"https://a.fsdn.com/allura/p/nagios/icon?1579016798?&w=90\",
    \"tts\":false,
    \"embeds\":[{
            \"title\":\"${NAGIOS_SERVICEDESC} on ${NAGIOS_HOSTNAME} ${NAGIOS_SERVICESTATE} (${NAGIOS_NOTIFICATIONTYPE})\",
            \"type\":\"rich\",
            \"description\":\"\",
            \"url\":\"${NAGIOS_DOMAIN}/cgi-bin/extinfo.cgi?type=2&host=$(echo "${NAGIOS_HOSTNAME}" | sed s/\ /\+/g)&service=$(echo "${NAGIOS_SERVICEDESC}" | sed s/\ /\+/g)\",
            ...
            \"fields\":[
                ...
                {\"name\":\"Service Name\",\"value\":\"${NAGIOS_SERVICEDESC}\",\"inline\":false},
                {\"name\":\"Service State\",\"value\":\"OK\",\"inline\":false},
                {\"name\":\"Service Data\",\"value\":\"${NAGIOS_SERVICEOUTPUT}\",\"inline\":false},
